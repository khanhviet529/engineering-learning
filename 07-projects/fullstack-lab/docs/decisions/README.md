...
- [ADR-0012: `packages/mock` — mock HTTP dựng từ hợp đồng, dùng chung cho dev và test](ADR-0012-contract-mock-package.md) — **`Proposed`, chờ chủ dự án ký.** Thêm workspace package thứ tư để frontend chạy được trước backend ở M2–M4; mock không cưỡng chế quyền, concurrency hay idempotency và không được deploy.
