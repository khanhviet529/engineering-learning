...
- [ADR-0014: Segment của URL viết bằng tiếng Việt không dấu, query key giữ tiếng Anh](ADR-0014-route-language.md) — URL là chữ người dùng gõ, nên nó theo ngôn ngữ của UI; API path và Screen ID không đổi. Ghi lại sau M2, vì tài liệu chưa từng có nơi công bố path thật.
- [ADR-0016: `code` là hợp đồng, `message` là chẩn đoán, và chữ cho người dùng thuộc frontend](ADR-0016-error-code-is-contract-message-is-ui.md) — Backend trả *điều gì đã xảy ra*; frontend quyết định *nói thế nào*. Dữ liệu động đi trong `details`, không trong văn xuôi. Ngoại lệ là một danh sách đóng, đếm được.
- [ADR-0017: File export tiến độ nằm trong PostgreSQL ở Phase 1.1](ADR-0017-report-export-file-storage.md) — Bytes ở bảng riêng `report_export_files`, không thêm service thứ năm. Chọn vì backup đã có sẵn và vì xoá file nằm cùng transaction với đổi status, nên không có trạng thái "row nói `ready` mà file đã mất".
- [ADR-0018: Sinh file export là đồng bộ ở Phase 1.1, và `202` là lời hứa không giữ được](ADR-0018-export-generation-is-synchronous-in-phase-1-1.md) — Hợp đồng mô tả một vòng đời bất đồng bộ trong một phase cấm cơ chế bất đồng bộ. Sửa **mã trạng thái**, không sửa vòng đời: `201` mang trạng thái thật, và client rẽ nhánh theo `status` chứ không theo mã.
- [ADR-0015: Tầng trong module `apps/api` được dựng theo việc nó phải làm, không theo quy tắc đồng loạt](ADR-0015-module-layering-scope.md) — Bốn tầng được biện minh bởi bốn port đang gánh việc thật, không bởi bản thân việc phân tầng; module không có việc thì dùng ít tầng hơn, như `activity` đang làm.
