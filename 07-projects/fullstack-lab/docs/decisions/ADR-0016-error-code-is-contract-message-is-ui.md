...
Khoá để chọn chữ vì vậy là **màn hình/đường vào**, không phải loại resource. Hai `404` về cùng một task — một trên board, một từ một deep link — cần hai câu khác nhau, và loại resource không phân biệt được hai ca đó. Khoá này cũng **miễn phí** theo mục 6: chữ sống trong module của feature đang gọi, nên `features/tasks/messages.ts` đã biết mình nói về task mà không cần ai gửi kèm thông tin đó.
...
| Server trả thêm `resource` (`{ code: "NOT_FOUND", resource: "TASK" }`) để frontend map theo `(code, resource)` | Một `code` generic vẫn phân biệt được resource; nghe như bù đúng chỗ thiếu | **Frontend đã có thông tin đó.** `NOT_FOUND` là code generic dùng cho "mọi endpoint project-data", và một `404` trả về từ `getTask(id)` là về task **theo cấu trúc của lời gọi** — server sẽ chỉ nói lại cho client điều client vừa nói với server. Đã kiểm mọi route trả `404`: không route nào mơ hồ về resource; kể cả `POST /tasks/:taskId/move`, vì `destinationColumnId` sai là `400 VALIDATION_FAILED` kèm `details[].field`, không phải `404`. Thêm nữa, khoá đúng là **màn hình** chứ không phải resource: cùng `resource: "TASK"`, một `404` trên board và một `404` từ deep link cần hai câu khác nhau, nên `resource` là khoá **yếu hơn** thứ frontend đã dùng. Và nếu một ngày thật sự cần, chỗ của nó là `details` của đúng code đó theo mục 3 — không phải một field thứ tư ngang hàng `code`/`message`/`details` cho **mọi** response. Cuối cùng là chỗ đáng cảnh giác: `404` ở đây **cố ý mơ hồ** ("hoặc không tồn tại, hoặc bạn không được thấy" — Workspace Admin chưa có `project_members` row nhận `404` chứ không `403`). Với route một-resource thì `resource` không rò gì thêm, nhưng nó **lập một khuôn mẫu**, và khi có route mà resource không suy ra được từ đường gọi thì cùng khuôn mẫu đó sẽ trả lời đúng câu hỏi mà `404` tồn tại để không trả lời — bảng error code viết thẳng "**không** suy ra resource có tồn tại" |
...
- Xuất hiện một route mà `404` **thật sự mơ hồ** giữa hai resource và không dùng được `400` field-level: đó là lúc `details: { resource }` cho đúng code đó được xét lại — nhưng vẫn không phải một field top-level, và vẫn phải cân với việc `404` cố ý không nói resource nào tồn tại.
