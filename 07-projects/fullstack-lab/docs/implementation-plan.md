...
| Secret local là plaintext trong `.env` trên đĩa (`CURSOR_SECRET`, `CSRF_SECRET`, mật khẩu database) | **Nợ, để M5** | `.gitignore` chặn việc commit, không chặn việc file tồn tại và bị đọc. Production phải lấy chúng từ secret manager, không từ file. Cơ chế đọc config đã đúng hình (xem dưới), chỗ chưa đúng là cách giữ secret |
...
| Tên `SESSION_SECRET` là misnomer: nó ký **cursor**, không ký session | **Đã trả 06/09/2026** | Nay là `CURSOR_SECRET` / `CURSOR_SECRET_PREVIOUS`. Tên cũ vẫn được nhận trong khoảng chuyển tiếp kèm **một** cảnh báo gộp cho cả bảng — hai dòng cho cùng một việc phải làm là hai dòng người ta học cách bỏ qua. Chưa xoá tên cũ: một lần deploy mà mọi môi trường phải đổi biến đồng thời là một lần deploy sẽ có môi trường bị bỏ quên. Chính cái tên này khiến **tôi** viết sai bài toán rotate |
...
| Không cắm được provider email thật | **Đã sửa 06/09/2026** | Ba chỗ mở theo lối optional; thiếu credential thì hành vi y hệt. Và **mô tả của tôi sai chiều**: tôi viết `.strict()` khiến app *từ chối khởi động* khi có `SMTP_USER`. Không phải — `loadEnv` **lọc** source xuống đúng key có trong schema **trước khi** parse, nên `.strict()` không bao giờ thấy key lạ. Đặt `SMTP_USER` trước hôm đó sẽ bị **bỏ qua im lặng**: app chạy bình thường, gửi mail **không xác thực**, người vận hành tin là đã xác thực. Chiều của sai lệch quan trọng — *từ chối khởi động* là lỗi ồn và tự sửa; *im lặng bỏ credential* là đúng loại lỗi codebase này được dựng để chặn |
...
| `PRM-01` được hứa "danh sách ứng viên đã là member workspace" mà không endpoint nào cấp cho actor cần nó | **Đã quyết và viết hợp đồng 06/09/2026** — `GET /projects/:projectId/member-candidates`, quyền `project:member:manage`. Chủ dự án chọn **mở**; chờ backend hiện thực | Lần thứ ba danh mục màn hình hứa dữ liệu không tồn tại (sau count của `BRD-02` và task nhúng của `GET /projects/:projectId`). Nhưng lần này **khác về bản chất**: `GET /workspaces/:id/members` đòi `workspace:member:manage` (Workspace Admin), còn `PRM-01` là **Owner** của project — hai vai khác nhau. Mở một route ứng viên cho Owner là **nới ranh giới nhìn thấy**: hôm nay một workspace member thường **không** xem được danh sách thành viên workspace. Đó là quyết định bảo mật, không phải một endpoint thiếu |
...
| **CI chưa từng chạy trong sáu mốc** | **Đã sửa 05/09/2026** | Hai lý do. Chưa push — 94 commit chỉ tồn tại trên một máy. Và kể cả push cũng không chạy: GitHub Actions **chỉ đọc** `.github/workflows/` ở **gốc repository**, còn file nằm ở `07-projects/fullstack-lab/.github/workflows/ci.yml`, nơi nó chỉ là một file YAML. Năm cổng mà mọi mốc coi là chặn merge đã là **văn bản** suốt thời gian đó. Nay ở `.github/workflows/fullstack-lab-ci.yml` kèm `paths` filter |
| `--fb-color-text-subtle` ship màu **không đạt** tương phản dù design đã sửa | **Đã sửa 06/09/2026** | `tokens.css` là file **được sinh ra**; vòng design v0.4 đổi `#94A3B8 → #6B7280` trong artifact, freeze chốt, `.pen` commit — **không ai chạy `pnpm tokens`**. Ứng dụng render màu 2,6:1 qua hai commit trong khi biên bản freeze ghi "đã sửa". Bắt được bởi bước CI viết **chính xác cho tình huống này** và chưa từng thực thi. Danh sách `KNOWN_UNREADABLE` nay **rỗng** |
| `dist` thiếu trên môi trường bắt đầu từ số không — **ba lần, ba chỗ** | **Đã sửa 06/09/2026** | `web.Dockerfile` (M5.5, FE tìm tay) · job `integration` (CI lượt 1) · job `quality` typecheck trước build (CI lượt 2). Cùng nguyên nhân: `packages/*` trỏ `main`/`types` vào `./dist`, và máy dev **luôn** có `dist` còn sót lại. Nếu xuất hiện **lần thứ tư** thì vấn đề nằm ở thiết kế package chứ không ở từng job, và lúc đó phải xét cho `exports` trỏ thẳng vào source |
| `client.setQueryData(key, undefined)` gọi **từ trong** `queryFn` — việc bỏ trang chưa bao giờ hoạt động | **Đã sửa 06/09/2026** | React Query lấy giá trị hàm trả về rồi nối vào danh sách trang **ngay sau đó**, nên lệnh xoá bị chính lần fetch gọi nó ghi đè. Màn hình báo "Danh sách đã thay đổi" rồi giữ nguyên cả 50 dòng cũ, gồm dòng của project actor vừa bị gỡ. Chuyển sang `useEffect` chạy **sau** khi fetch xong. Lộ ra khi bài `dead-cursor` đi được qua bước gỡ member — tức là **sửa một cổng bị chặn làm lộ thứ đứng sau nó**, lần thứ hai sau `500` của `PATCH /tasks` |
| Test M5 canh việc bỏ trang là một **khẳng định rỗng** | **Đã sửa 06/09/2026** | Nó hỏi *"dòng của trang đầu còn trên màn không"* — câu trả lời là **có** ở cả hành vi đúng lẫn sai, nên nó không bao giờ đỏ được. Nay hỏi *"nút `Tải thêm` có quay lại không"* — chỉ đúng khi các trang thật sự đã bị bỏ. Gỡ bản sửa ra thì đỏ. Đây là lần thứ ba một khẳng định rỗng bị bắt trong dự án này |
| `dist` thiếu — **lần thứ tư**, và điều kiện tôi đặt ra đã kích hoạt | **Cần quyết ở vòng sau** | Lần này: `drizzle-kit migrate` nạp `drizzle.config.ts` → schema của `apps/api` → `@flowboard/contracts`. Tôi từng viết *"lần thứ tư thì phải xét cho `exports` trỏ thẳng vào source"* — **và đó là bản sửa sai**: `apps/api` chạy production bằng `node dist/main.js`, nên `main` trỏ `.ts` sẽ không nạp được. Bản sửa đúng hẹp hơn: chỉ có **ba** loại điểm vào chạy từ checkout sạch (job CI, Dockerfile, máy dev), và cả ba phải build `packages/**` trước. Cần một guard đọc thực tại quét chúng — hoặc chấp nhận rằng nó sẽ tái diễn |
| `execFileSync` trong `e2e/src/db.ts` chặn event loop, làm đồng hồ Playwright đứng | **Đã sửa 06/09/2026** | Spec treo 10 phút **không in một dòng nào, không đỏ, không hết giờ**. Một bộ kiểm không thể hết giờ thì không phân biệt được "đang chạy" với "đã chết". Nay có timeout và một câu ném đọc được |
| Bốn cặp token trùng nhau còn lại trong namespace `fb.color.text.*` | **Nợ design v0.6** | `text.placeholder` = `text.muted` **tuyệt đối ở cả hai theme**; `text.soft` = `text.muted` ở sáng; `text.secondary-alt` ≈ `text.secondary` ở sáng; và `text.faint` cho **1,6:1** trên nền trắng — không hợp lệ làm chữ nhưng vẫn nằm trong namespace `text.*`. Design nêu ra khi gộp `subtle`, và lập luận của họ đúng: tạo `icon.*` cho một token trong khi `text.faint` mắc **đúng cùng lỗi đặt tên** là sửa nửa vời |
| Sau khi gộp, palette **không còn** màu foreground nhạt hợp lệ nào | **Nợ design v0.6** | Design nói trước hệ quả: người tiếp theo muốn chevron nhạt hơn sẽ không có token để với, và đường tắt là **ghi cứng hex** — hiện đang là 0. Vòng sau nên tạo nhóm màu trang trí có tên **không** bắt đầu bằng `text.` |
| `packages/ui/dist/data-display.js` cũng mang `var(--fb-color-text-subtle)` | **Đã sửa 06/09/2026** | Design phát hiện: đây là artifact build **thứ hai** không nằm trong cổng `pnpm tokens && git diff --exit-code` — cổng đó chỉ canh `tokens.css`. `dist/` không được commit nên chưa thành `tokens.css` thứ hai chờ lệch, nhưng đáng biết trước khi v0.6 xoá biến |
| Một lượt `pnpm verify` đỏ 1/798 ở `apps/api` mà **không xác định được bài nào** | **Đã định danh 06/09/2026** — `CONNECT_TIMEOUT localhost:5433`, xem dòng dưới | Tôi lọc output lượt đó bằng `grep` và lọc mất chính dòng tên bài. Ba lượt sau đó — hai lượt `apps/api` riêng và một lượt `verify` đầy đủ — đều 798/798. Ghi lại thay vì bỏ qua: một cổng đỏ ngẫu nhiên là **bằng chứng**, và cùng lớp nguyên nhân (oversubscription, ngưỡng chờ) đã cắn ba workspace. Việc phải làm là **không lọc** output của cổng: một lệnh chạy nền phải giữ nguyên dòng thất bại, vì lúc cần nó thì không chạy lại được |
| Container PostgreSQL chạy lâu làm `apps/api` đỏ bằng `CONNECT_TIMEOUT`, và **ba cách sửa hiển nhiên đều sai** | **Nợ, mở 06/09/2026** | Bốn lượt liên tiếp đỏ với số file hỏng tăng đều 1 → 2 → 7 → 10, chữ ký luôn là `write CONNECT_TIMEOUT localhost:5433` sau 30s. Ba giả thuyết bị **phép đo bác bỏ**, không phải bị suy luận bác bỏ: dừng container `api`/`web` làm đỏ **nhiều hơn**; `pg_stat_activity` peak **11/100** nên không cạn connection; `netstat` peak **1** socket tới `:5433` và 11 `TIME_WAIT` nên không cạn ephemeral port. `--fileParallelism=false` cũng không cứu — vẫn 2 file đỏ, và tốn 566s thay vì 200s, nên nó bị gỡ chứ không được giữ "cho chắc". Thứ đổi kết quả là `docker restart flowboard-postgres-1`: lượt ngay sau đó **815/815** song song đầy đủ trong 239s. Cơ chế **chưa chứng minh được** — một data point, và máy vẫn chỉ còn 0,53 GB RAM trống. Còn nợ: một cách để phân biệt "database hỏng" với "đường tới database hỏng" ngay trong thông điệp lỗi, vì `CONNECT_TIMEOUT` không nói được điều đó và tôi đã tốn năm lượt chạy để đuổi sai hướng |
| Tôi nói "đã sửa cùng lượt" trong khi ba commit **chưa rời máy tôi** | **Đã sửa 19/09/2026** | Backend dừng M6 lần thứ hai vì `endpoint-contracts.md` và `progress-export.md` ở checkout của họ vẫn ghi `202`. Họ đúng: `origin/main` còn ở `5cca8e2`, còn ADR-0017, ADR-0018 và schema M6 nằm trong ba commit **chưa push**. Hợp đồng mà agent đọc là hợp đồng trên `origin`, không phải hợp đồng trong working tree của tôi — và suốt hai vòng tôi viết prompt như thể hai thứ đó là một. Cùng lớp sai lầm với `tokens.css`: tin vào một **mô hình** của thứ thật thay vì thứ thật. Việc phải làm: một lệnh giao prompt không được tách khỏi lệnh push, và câu "tài liệu ở X" trong prompt phải kèm commit hash |
| Dòng `requested` mồ côi khi API chết giữa hai transaction | **Nợ có tên, mở 19/09/2026** | [ADR-0018](decisions/ADR-0018-export-generation-is-synchronous-in-phase-1-1.md) sinh file đồng bộ, nên cửa sổ đúng bằng thời gian render một file — nhưng không gì lật được dòng đó. Sửa ở Phase 1.1 nghĩa là dựng đúng cái worker đang bị cấm. Hậu quả bị chặn: dòng đó không bao giờ tải được (`409 REPORT_NOT_READY`), nên nó tốn dung lượng chứ không gây hiểu nhầm; người dùng tạo request mới |
| Bytes của export **không** biến mất sau `expires_at` | **Nợ có tên, mở 19/09/2026** | [ADR-0017](decisions/ADR-0017-report-export-file-storage.md) chọn để file trong PostgreSQL. `expires_at` chặn được việc **tải**, nhưng Phase 1.1 chưa có gì chạy định kỳ để `DELETE` dòng trong `report_export_files`, nên dung lượng chỉ tăng. Đây là chỗ đánh đổi đã biết chứ không phải chỗ bỏ sót: lịch purge cần một thứ chạy nền, và thứ chạy nền là Phase 1.2. Tín hiệu phải xem lại: bảng vượt một ngưỡng dung lượng đo được, hoặc Phase 1.2 mở |
| Job `integration` của CI **có thể xanh mà không chạy một integration test nào** | **Đã sửa 06/09/2026** | Backend nêu nghi vấn nhưng không chạm được `.github/`; tôi đọc log CI thật và xác nhận bằng số: job `quality` báo `434 passed | 334 skipped (768)` và **xanh**, vì `describeIfDb` bỏ qua khi thiếu URL. Job `integration` mất `DATABASE_URL_HOST` sẽ báo đúng con số đó và cũng xanh — một phần ba bộ test ngừng kiểm mà không ai biết. Nay `REQUIRE_DB=1` chỉ đặt ở job đó, và thiếu URL **ném** thay vì skip. Máy dev không đặt cờ nên vẫn skip như cũ |
| Biến rỗng phá đường tương thích ngược của việc đổi tên | **Đã sửa 06/09/2026** | Compose phải truyền cả hai tên với default `:-`, và khi đó nó đặt chúng thành `""` trong container. `""` không phải `undefined` nên nó rơi vào `min(32)` và **giết mọi môi trường local**. Backend tự phát hiện; không có bước này thì đường tương thích ngược chỉ đúng **trên giấy** |
| `.gitignore` suýt nuốt mất `.env.production.example` | **Đã sửa 06/09/2026** | `.env.*` ignore mọi thứ với allowlist đúng hai file. File mới tồn tại trên máy backend, test xanh, repo **trống rỗng**. Câu của họ đáng giữ: *"một file không bao giờ được commit cũng là một file không bao giờ được kiểm"* |
| `packages/ui` chạy ở `testTimeout` mặc định 5s | **Đã sửa 06/09/2026** | Workspace **thứ ba** cần đúng bản chỉnh này, sau `apps/api` (M4) và `apps/web` (M5). Bài `nhãn trỏ tới input` dùng `userEvent` và hết giờ khi cả bộ chạy song song. Tôi thêm cả `fileParallelism: false` **theo suy diễn** rồi đo: 61s thành 293s, gấp năm, đổi lấy một thứ chưa ai chứng minh là cần — đã gỡ. `apps/web` tắt nó vì frontend **đo được** flake thật |
