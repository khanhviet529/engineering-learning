...
| Không có kiểm tự động nào khẳng định `start` không nạp file env | **Nợ, để M5** | Quy tắc "`dev` được nạp file, `start` thì không" nay đã ghi ở [local development](operations/local-development.md), nhưng hôm nay nó đúng vì có người viết đúng. Một test đọc `package.json` sẽ làm nó đúng vì **không thể sai** — cùng lối `web-routes.test.ts` đọc `src/app` trên đĩa |
