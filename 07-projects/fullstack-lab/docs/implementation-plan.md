...
| Không có cửa sổ rotate cho `SESSION_SECRET` | **Nợ, để M5** | Đổi key hôm nay là vô hiệu mọi session đang mở, vì tất cả đang ký bằng key cũ. Rotate mà không đá ai ra cần **hai** key trong một khoảng: ký bằng key mới, verify bằng cả hai. Đây là thiết kế chứ không phải cấu hình, nên nó là việc của một mốc chứ không phải một dòng config |
...
| `workspaces` không có cột `timezone`, dù 5 chỗ trong tài liệu nói `dueState` theo timezone workspace | **Đã che bằng port 05/09/2026, cột vẫn là nợ** | Backend **không tự thêm cột** — thêm cột là tự phát minh hợp đồng. Thay bằng port `WorkspaceClock` + `APP_TIMEZONE` (mặc định `Asia/Ho_Chi_Minh`). Khi cột thật có, chữ ký thành `today(workspaceId)` và không use case nào phải sửa. Cùng khuôn mẫu `ColumnEmptinessCheck` |
