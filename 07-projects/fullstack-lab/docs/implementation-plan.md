...
| Một lượt `pnpm verify` đỏ 1/798 ở `apps/api` mà **không xác định được bài nào** | **Nợ, mở 06/09/2026** | Tôi lọc output lượt đó bằng `grep` và lọc mất chính dòng tên bài. Ba lượt sau đó — hai lượt `apps/api` riêng và một lượt `verify` đầy đủ — đều 798/798. Ghi lại thay vì bỏ qua: một cổng đỏ ngẫu nhiên là **bằng chứng**, và cùng lớp nguyên nhân (oversubscription, ngưỡng chờ) đã cắn ba workspace. Việc phải làm là **không lọc** output của cổng: một lệnh chạy nền phải giữ nguyên dòng thất bại, vì lúc cần nó thì không chạy lại được |
