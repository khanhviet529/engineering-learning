...
| Elevation | `fb.shadow.raised`, `fb.shadow.drawer`, `fb.shadow.modal`, `fb.shadow.color` | Cấp nổi của card và lớp phủ, **ba cấp đều đã hiện thực trong artifact**. Ba cấp khác nhau ở cả alpha lẫn hình học, không chỉ ở độ đậm: `fb.shadow.raised` `#0F172A12` — card trên nền trang, offset `(0, 2…6)`, blur nhỏ. `fb.shadow.drawer` `#10182826` — panel neo cạnh phải, offset **ngang** `(-12, 0)` blur 24, vì drawer cao hết màn nên chỉ cạnh trái đổ bóng. `fb.shadow.modal` `#10182833` — hộp thoại canh giữa trên scrim, offset `(0, 12)` blur 40. Giữ ba cấp tách biệt vì `fb.z.drawer` (300) thấp hơn `fb.z.modal` (400): khi dialog xác nhận mở trên một drawer đang mở, bóng là tín hiệu phân lớp duy nhất. |
