...
| PRM-01 | Project Members — trang con | Để Owner quản lý membership và vai trò của project hiện tại. | `/du-an/:projectId/thanh-vien` | thành viên project/role, `GET /projects/:projectId/member-candidates` cho danh sách ứng viên (`userId`, `displayName`, `email` — workspace member chưa ở trong project), capability quản lý project member | Owner | Loading, Empty, Error, Forbidden, mutation pending, điều kiện chặn “chưa là thành viên workspace”. |
