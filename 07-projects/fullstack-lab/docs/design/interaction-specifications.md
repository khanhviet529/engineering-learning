...
- `evidenceUrl` là optional ở mọi cột. Client validate sớm scheme `https` và độ dài 2048 để phản hồi nhanh, nhưng server là quyết định cuối; `http`/`javascript:`/`data:` hiển thị lỗi tại field. Task Detail render liên kết kèm host dạng text, mở tab mới với `rel="noopener noreferrer"`, và **không** tải preview/thumbnail của URL.
- Comment composer có thể có toolbar định dạng, nhưng nó chỉ chèn cú pháp Markdown thuộc subset đã allowlist; không có chế độ WYSIWYG lưu HTML, không paste-as-HTML, không upload ảnh. Comment đã gửi vẫn bất biến.
