...
| Core MVP | Đặt `due_date` quá hạn cho một task rồi move vào cột `is_terminal = true`: `dueState` chuyển `none` và task rời khỏi filter `overdue`; move ngược ra cột không terminal thì `overdue` xuất hiện lại và activity có **đúng một** `task.reopened` (không kèm `task.moved`). | [ADR-0008](../decisions/ADR-0008-terminal-column-and-task-reopen.md), [database design](../data/database-design.md) |
