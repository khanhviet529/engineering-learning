...
Task and board-column positions use gap/fractional ordering, so ordinary drag-and-drop does not rewrite every row. If fractional values become too dense, the server runs a controlled rebalance inside a transaction with locks on affected rows, preserves deterministic order and uniqueness, and returns the committed task/column state. Rebalance is server behavior, not a public bulk-table reorder API.
