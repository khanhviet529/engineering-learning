...
Yêu cầu `project:read`. Không cần query để mở board: `200` trả project projection, current-project capabilities, active column và assignable project-member projection — đúng `projectDetailSchema`: `{ project, capabilities, columns, members }`. **Không có task trong response này.** `columns` từ M3; ở M2 nó là mảng rỗng, vì trả mảng rỗng là trung thực còn bỏ hẳn field sẽ buộc client viết hai nhánh cho cùng một endpoint.

Bản trước của dòng này hứa "page đầu default 25 task độc lập cho mỗi column", nhưng `projectDetailSchema` chưa bao giờ có chỗ đặt chúng — hợp đồng tự mâu thuẫn từ lúc được viết, và frontend phát hiện khi dựng board ở M4. Sửa theo hướng **bỏ lời hứa** thay vì thêm field, vì đây là chiều đảo được: thêm một field optional về sau là additive và không phá client nào, còn bỏ một field đã nhúng thì phá. Client vì vậy gọi `GET /projects/:projectId/tasks?columnId=…` một lần cho mỗi column ở lần vẽ đầu — N request, đã ghi thành mục hiệu năng của M5. Board load bình thường loại archived column và không bao giờ fetch toàn bộ task của project lớn. Load-more của một column dùng explicit task-list use case `GET /projects/:projectId/tasks?columnId=...&cursor=...`, nên không ảnh hưởng page của column khác. ID non-member/cross-project trả `404`; member nhìn thấy project nhưng thiếu action trả `403`.
...
Yêu cầu `task:move`, CSRF và `Idempotency-Key`. Body đúng shape `{ "destinationColumnId", "targetPosition", "expectedVersion", "reviewerId?" }`. `targetPosition` là **gợi ý vị trí**, không phải một giá trị bắt buộc phải do server cấp trước đó: [ADR-0006](../decisions/ADR-0006-fractional-ordering-and-concurrency.md) mục 1 nói server tính mọi giá trị và chỉ dùng giá trị của client như gợi ý đã được validate lại. Khi column đích **rỗng** thì không có giá trị nào từng được cấp và không có neighbour nào để chen giữa, nên server **bỏ qua** gợi ý và tự tính; client gửi một decimal hợp lệ bất kỳ. Client không bao giờ hiển thị hay tính toán trên `position`. `reviewerId` chỉ được gửi/khi cần nếu destination column có `requiresReviewer`; nếu cột này yêu cầu reviewer, server bắt buộc ProjectMember reviewer khác assignee. `200` trả committed task với destination/position/version/dueState mới. Transaction validate active same-project destination, chỉ lock order range cần thiết, rebalance khi cần và ghi đúng một `task.moved` activity. Version stale là `409` đã định nghĩa; không có force move. Khi cột nguồn có `isTerminal = true` và cột đích có `isTerminal = false`, move này là **mở lại task**: transaction ghi đúng một activity `task.reopened` thay cho `task.moved` (vẫn đúng một activity cho một move commit), không cần permission, endpoint hay body field nào khác, và mọi validation reviewer/version/idempotency áp dụng như move thường.
...
Chỉ Phase 1.1. Yêu cầu `report:export` (Owner), CSRF và `Idempotency-Key`. Body đúng shape `{ "filters" }`, trong đó `filters` là task-filter subset canonical đã allowlist và project-scoped; không raw query/table/column. Subset đó dựng **từ** `taskFilterFields` của `packages/contracts`, không chép lại, và loại đúng hai field: `sort` (thứ tự dòng do server quyết — để client chọn thì hai lần export cùng dữ liệu cho hai file khác nhau, và lời hứa "cùng canonical payload → cùng record" của idempotency biến thành một lời hứa về thứ tự dòng) và `columnId` (một báo cáo **tiến độ** nói về cả bàn; lọc còn một cột thì phần "tiến độ" không còn nghĩa). `201` trả **cùng một** shape report với `GET /reports/:reportId` — `{ id, projectId, status, fileName, contentType, byteSize, expiresAt, createdAt, updatedAt }` — mang trạng thái **thật** tại thời điểm trả về, và ba field mô tả file là `null` khi chưa có file. Một tài nguyên **một** shape: hai shape cho cùng một thứ là hai đường để client sai, và cái sai đó chỉ lộ ra ở trạng thái `ready`, tức là muộn. **`201` chứ không `202`, và client không được rẽ nhánh theo mã này** — [ADR-0018](../decisions/ADR-0018-export-generation-is-synchronous-in-phase-1-1.md). Phase 1.1 sinh file **đồng bộ** trong chính request, nên xử lý đã xong lúc response rời server và `202` ("đã nhận, xử lý chưa xong") sẽ là một phát biểu sai về hệ thống. Status trong body vì vậy là `ready` hoặc `failed`, **không bao giờ** `requested`. Phase 1.2 có worker sẽ đổi mã thành `202` và status khởi đầu thành `requested`; client đọc `status` trong body và phải không hỏng vì lần đổi đó. Hai transaction, không gộp. Transaction một: authorize, validate và snapshot filter, insert với `requested`, ghi `report_export.requested`, **commit**. Rồi render, ngoài mọi transaction — claim phải commit trước khi làm việc tốn thời gian, đúng khuôn mẫu idempotency. Transaction hai: insert bytes, chuyển status. Render có **ngân sách thời gian tường minh**; vượt ngân sách là `failed`, không phải một request treo. Editor/Viewer/non-member Workspace Admin không thể request.

**Bộ lọc export không thừa hưởng từ màn hình nào.** `RPT-01` sở hữu bộ lọc của riêng nó, mặc định `{}` — "Tất cả công việc". Cụ thể, cửa sổ `from`/`to` của `GET /projects/:projectId/overview` **không** được map sang `filters.dueFrom`/`dueTo`, và map như vậy sai hai lần: sai **cột**, vì cửa sổ overview lọc theo `tasks.created_at` còn `dueFrom`/`dueTo` lọc theo `dueDate` — người dùng thấy một khoảng ngày trên màn hình rồi export và nhận một tập task khác, không có cách nào biết; và sai **phạm vi**, vì cửa sổ đó chỉ ảnh hưởng `totals.createdInWindow` trong khi `byColumn`, `byAssignee` và `dueStates` vẫn là cả project — đẩy nó thành bộ lọc của cả file là biến một chú thích thành một điều kiện lọc. Đó là lý do `reportFilterSchema` có đủ chín field: nó được thiết kế để người dùng đặt, không để kế thừa.

### GET /reports/:reportId — xem trạng thái export

Chỉ Phase 1.1. Yêu cầu `report:export`; `200` trả report metadata an toàn, đúng shape của `202` ở trên; `fileName`/`contentType`/`byteSize` là `null` cho tới khi `ready`. `status` là giá trị server ghi (`requested|ready|failed|purged`); trạng thái "expired" trong UI là **derived từ `expiresAt`**, không phải một status ghi trong database — client so `expiresAt` với thời điểm hiện tại để hiển thị và disable download. Không bao giờ trả `fileStorageKey`, filter snapshot của project khác hay download URL. Hidden project/report trả `404`; report hết hạn được biểu diễn/xử lý mà không lộ usable file.
