...
Chỉ Phase 1.1. Yêu cầu `report:export` (Owner), CSRF và `Idempotency-Key`. Body đúng shape `{ "filters" }`, trong đó `filters` là task-filter subset canonical đã allowlist và project-scoped; không raw query/table/column. Subset đó dựng **từ** `taskFilterFields` của `packages/contracts`, không chép lại, và loại đúng hai field: `sort` (thứ tự dòng do server quyết — để client chọn thì hai lần export cùng dữ liệu cho hai file khác nhau, và lời hứa "cùng canonical payload → cùng record" của idempotency biến thành một lời hứa về thứ tự dòng) và `columnId` (một báo cáo **tiến độ** nói về cả bàn; lọc còn một cột thì phần "tiến độ" không còn nghĩa). `202` trả **cùng một** shape report với `GET /reports/:reportId` — `{ id, projectId, status, fileName, contentType, byteSize, expiresAt, createdAt, updatedAt }` — với `status: "requested"` và ba field mô tả file là `null`. Một tài nguyên **một** shape: hai shape cho cùng một thứ là hai đường để client sai, và cái sai đó chỉ lộ ra ở trạng thái `ready`, tức là muộn. `202` chứ không `201` vì request đã được nhận nhưng file **chưa** tồn tại; `201` sẽ khiến client tải ngay rồi nhận `409`. Transaction snapshot validated filters, ghi `report_export.requested`, và không giữ DB transaction khi generate file. Editor/Viewer/non-member Workspace Admin không thể request.
