...
- Comment render bằng Markdown renderer có **allowlist tường minh là contract, không phải tuỳ chọn của component**: bold, italic, inline code, fenced code block, unordered/ordered list, link. Raw HTML passthrough bị tắt tuyệt đối; không image, table, heading, blockquote, embed. Link chỉ nhận `https`, `http`, `mailto` và luôn có `rel="noopener noreferrer"`. Body vẫn là plain text nên client không render Markdown sẽ hiển thị ký tự cú pháp — degrade như vậy là chấp nhận được.
- `evidenceUrl` render như một link có `rel="noopener noreferrer"` kèm host dạng text để người đọc thấy đích trước khi bấm. **Không** fetch preview, thumbnail, favicon hay metadata của URL đó ở bất kỳ tầng nào.
