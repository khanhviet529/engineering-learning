...
2. **`ResourceProjectResolver`** resolve resource của route về project sở hữu nó. Project ID resolve trực tiếp; task resolve qua `task.project_id`; comment qua task của nó; column qua `column.project_id`; report qua `report.project_id`. Nó không bao giờ tin một `projectId` do client gửi mà mâu thuẫn với owner đã resolve được.
   ...
   Một route **không có resource trên path** thì được resolve từ body: `POST /columns/reorder` mang `projectId` trong body vì không có gì trên path để suy ra. Resolver vì vậy nhận cả `params` và `body`, và chiều ưu tiên giữ nguyên — **có resource trên path thì body không bao giờ ghi đè**. Nếu cả hai xuất hiện, path thắng. Đảo lại là mở một đường cho client tự khai project của một resource nó không sở hữu, đúng thứ nguyên tắc "`ID` là locator, không phải bằng chứng quyền" tồn tại để chặn.
