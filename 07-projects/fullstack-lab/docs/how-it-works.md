...
| Schema | Composite FK `(project_id, column_id) → board_columns(project_id, id)` chặn cross-project ngay tại DB; `UNIQUE (project_id, column_id, position)` DEFERRABLE; `version` tăng đúng một. | [Database design](data/database-design.md) |
