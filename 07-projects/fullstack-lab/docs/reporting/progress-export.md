...
`POST /projects/:projectId/reports/progress-export` yêu cầu session, CSRF, `report:export` và `Idempotency-Key`; response `201` trả đúng shape của `GET /reports/:reportId` — `{ id, projectId, status, fileName, contentType, byteSize, expiresAt, createdAt, updatedAt }` — với ba field mô tả file là `null` khi chưa có file. Status là `ready` hoặc `failed`, **không bao giờ** `requested`, vì Phase 1.1 sinh file đồng bộ trong chính request ([ADR-0018](../decisions/ADR-0018-export-generation-is-synchronous-in-phase-1-1.md)). Client rẽ nhánh theo `status` trong body, **không** theo mã trạng thái: Phase 1.2 sẽ đổi mã thành `202`. Cùng key, cùng actor và cùng canonical request phải trả cùng kết quả export thay vì tạo bản thứ hai. Cùng key với payload khác bị từ chối theo [idempotency contract](../api/pagination-concurrency-idempotency.md); key không là authorization claim và không được dùng làm ID report.
...
Phase 1.1 không đòi queue, và [ADR-0018](../decisions/ADR-0018-export-generation-is-synchronous-in-phase-1-1.md) đã **chốt** cơ chế thay vì để ngỏ như bản trước của mục này: sinh đồng bộ trong chính request, sau khi transaction claim commit, cập nhật trạng thái trong transaction riêng, không Redis/BullMQ/worker. Hai phương án bị loại đều nói sai một điều gì đó — trả DTO chụp lúc `requested` thì response nói sai về chính nó, còn fire-and-forget trong process API là một worker **không có recovery**, để lại dòng `requested` vĩnh viễn sau một lần restart. Khi thành công, server ghi file metadata rồi chuyển `requested → ready`; khi lỗi không recoverable, ghi safe error code và chuyển `requested → failed`. Không ghi raw query, credential, path nội bộ, payload task không cần thiết hay exception vào response/activity.
