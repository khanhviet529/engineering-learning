...
- **Chính sách ghi file (cập nhật 03/09/2026, thay nhịp checkpoint trước đó):** MCP của pen.dev chỉ tác động lên **document trong bộ nhớ editor**, không ghi đĩa; chỉ `Ctrl+S` của người vận hành mới ghi. Vì vậy:
  - Dòng design **không** yêu cầu save giữa việc và không chờ save để tiếp tục; nó làm liền mạch trong editor rồi báo cáo.
  - **Controller là bên save**, khi công việc đã hoàn chỉnh — và **bắt buộc save trước khi đóng editor hoặc reload window**. Đây chính là chỗ đã mất trắng Canvas v0.4: state bộ nhớ không phải file. Nếu muốn bỏ hẳn rủi ro này thì bật `files.autoSave` cho workspace.
  - **Kiểm chứng vẫn hai lớp:** số đo qua MCP là số của **bộ nhớ**; chỉ đĩa đổi mới chứng minh công việc đã landed. Bằng chứng đĩa là `git status --porcelain` thấy file ở trạng thái `M` cộng `git diff --stat`, **không** phải so `git hash-object` với blob cũ: `.pen` là JSON thuần nên git chuẩn hoá line ending khi add, làm hash thô của working file lệch với blob mà nội dung không đổi (đã gặp: 3.463.301 byte CRLF → 3.265.871 byte LF, cùng nội dung). `07-projects/fullstack-lab/.gitattributes` pin `*.pen text eol=lf` để hai phía không còn lệch vì lý do này. Báo cáo của dòng design phải nói rõ số của mình là đo trên bộ nhớ; controller gắn hash đĩa tại thời điểm save. Một claim chỉ được coi là đã landed khi hash đĩa đổi.

- **Kiểm chứng P0.2 (03/09/2026):** hash đĩa `469ef4b6492370c6111aa408dd6a9c608164a32c` (size 3.463.301) khớp đúng con số dòng design báo, và controller đã đo lại độc lập qua MCP: `nodes 7661 · depth0 135 · reusable 28 · refInstances 169 · hardcodedHexProps 0 · distinctHex 0 · variables 187 / fb. 156 · textNodes 2841 · fontSizeUnder11 0 · clipped 0`. **Cả mười chỉ số khớp chính xác**, nên P0.2 đạt: artifact có thật, token hoá 4362 → 0 là thật, componentisation 0 → 169 instance là thật. Bản tự khai phần chưa làm cũng đúng: 79 màn desktop cũ chưa chuyển sang instance, `isTerminal`/nhãn `Mở lại` chưa dựng, cặp `05/06 Authentication` cũ nay trùng vai với 20 trang AUTH mới, và chưa có số đo tương phản.
