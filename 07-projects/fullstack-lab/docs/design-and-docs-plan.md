...
- **Chính sách ghi file (cập nhật 03/09/2026, thay nhịp checkpoint trước đó):** MCP của pen.dev chỉ tác động lên **document trong bộ nhớ editor**, không ghi đĩa; chỉ `Ctrl+S` của người vận hành mới ghi. Vì vậy:
  ...
  - **Kiểm chứng vẫn hai lớp và không đổi:** số đo qua MCP là số của **bộ nhớ**; chỉ `git hash-object` đổi mới chứng minh **đĩa**. Báo cáo của dòng design phải nói rõ số của mình là đo trên bộ nhớ; controller gắn hash đĩa tại thời điểm save. Một claim chỉ được coi là đã landed khi hash đĩa đổi.
