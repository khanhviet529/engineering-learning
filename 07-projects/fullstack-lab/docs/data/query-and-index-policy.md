...
- Query fingerprint bind tối thiểu vào project, column khi có, `sprintId` khi có, `parentTaskId` khi có, filters, sort direction và search. Đổi `project_id`, `columnId`, filter, sort hoặc search sẽ reset cursor và tải page đầu; reuse cursor khác fingerprint bị từ chối.
...
|---|---|---|
| Filter | `columnId`, `assigneeId`, `createdById`, `reviewerId`, `category`, `priority`, `dueState`, `dueFrom`, `dueTo`, `sprintId` (Phase 1.4), `parentTaskId` (Phase 1.5) | UUID/date/enum parse trước query; `dueFrom <= dueTo`; column/user/sprint phải thuộc project; `dueState` do server suy ra. `sprintId` nhận UUID sprint cùng project hoặc token `backlog` cho `sprint_id IS NULL`; chỉ hợp lệ khi project đã bật Sprint. `parentTaskId` nhận UUID task cùng project để liệt kê subtask, hoặc token `none` cho task không có cha. |
...
- Task update chỉ allowlist `title`, `description`, `assignee_id`, `reviewer_id`, `category`, `priority`, `start_date`, `due_date`, `evidence_url`, `sprint_id` (Phase 1.4) và `parent_task_id` (Phase 1.5), cùng `expectedVersion`. `evidence_url` được validate scheme `https` cùng độ dài trước khi ghi và **không được server fetch**; nó không phải filter, không phải sort, không search nên cố ý không có index. Nó từ chối `project_id`, `column_id`, `created_by_user_id`, `position`, `version`, `due_state`, timestamps và audit fields. Đổi `column_id` hoặc `position` bắt buộc dùng dedicated Task move use case/transaction.
...
| `task_dependencies(project_id, blocking_task_id)` | btree | Danh sách task mà một task đang chặn, và chiều duyệt còn lại. | Phase 1.5 only |
...
| Task move | Dedicated transaction: re-check permission/scope/expected version; lock task, source/destination ordering rows theo nhu cầu; xác nhận destination active/cùng project và reviewer nếu destination yêu cầu; tính position, update column/position/version, rebalance có kiểm soát nếu cần; insert activity; commit. | Đúng một activity cho một move commit: `task.moved`, hoặc `task.reopened` khi cột nguồn `is_terminal = true` và cột đích `is_terminal = false`. Không bao giờ ghi cả hai. |
...
| Column terminal flag | Conditional update `is_terminal` theo `id` và `project_id` sau Owner authorization; không đụng `position`, `archived_at` hay task nào; insert activity; commit. `due_state` của task trong cột đó đổi ngay ở lần đọc kế tiếp vì nó là giá trị dẫn xuất. | `board_column.terminal_changed` chỉ sau commit. |
| Column reviewer requirement | Conditional update `requires_reviewer` theo `id` và `project_id` sau Owner authorization; không đụng `position`, `archived_at`, `is_terminal` hay task nào; insert activity; commit. Điều kiện này **không hồi tố**: nó chỉ áp cho create và move sau thời điểm đổi, nên task đang nằm trong cột mà thiếu `reviewer_id` vẫn ở nguyên — hồi tố sẽ biến một thao tác cấu hình thành một đợt vi phạm invariant hàng loạt. | `board_column.reviewer_requirement_changed` chỉ sau update hợp lệ. |
...
| Task dependency add (Phase 1.5) | Re-check `task:update` và project scope; lấy advisory transaction lock theo `(project_id)`; chạy recursive CTE kiểm tra `blocked_task_id` có tới được `blocking_task_id` hay không; validate giới hạn 50 cạnh mỗi chiều; insert cạnh; insert activity; commit. Cycle trả `409 TASK_DEPENDENCY_CYCLE`, vượt giới hạn trả `400 VALIDATION_FAILED`, không cạnh nào được ghi. | `task_dependency.added` chỉ sau commit. |
| Task dependency remove (Phase 1.5) | Re-check `task:update` và project scope; `DELETE` cạnh theo `id` + `project_id`; insert activity; commit. Không cần advisory lock vì bỏ cạnh không thể tạo cycle. | `task_dependency.removed` chỉ sau commit. |
| Task parent change (Phase 1.5) | Trong task update transaction: xác nhận cha cùng project, khác chính task, và **cha không có cha** (giới hạn một cấp); reject nếu task đang là cha của task khác mà lại được gán cha. | `task.updated` như mọi thay đổi nội dung. |
