...
| Vì sao thứ tự async không như tôi nghĩ? | event loop, microtask, Promise | [01](../01-web-frontend/01-javascript-typescript/01-event-loop-async.md) |
| Vì sao hàm của tôi đọc giá trị cũ? | closure, scope | [03](../01-web-frontend/01-javascript-typescript/03-execution-context-closure.md) |
| UI phản ứng khi state thay đổi thế nào? | React render, reconciliation | [01](../01-web-frontend/02-react/01-state-render.md) [05](../01-web-frontend/02-react/05-reconciliation-keys.md) |
| Dữ liệu async đồng bộ với UI thế nào? | effect, cleanup, AbortController, TanStack Query | [02](../01-web-frontend/02-react/02-effects-lifecycle.md) [03](../01-web-frontend/02-react/03-async-race-condition.md) [04](../01-web-frontend/02-react/04-server-state-cache.md) |
| Server và client chia việc thế nào? | Next.js RSC, hydration | [01](../01-web-frontend/03-nextjs/01-server-client-boundary.md) [04](../01-web-frontend/03-nextjs/04-rendering-strategies.md) |
| Vì sao trang không cập nhật dù DB đã đổi? | Next.js cache, revalidate | [03](../01-web-frontend/03-nextjs/03-data-fetching-cache.md) |
| Một HTTP request đi qua backend ra sao? | HTTP, Node.js, NestJS pipeline | [01](../02-backend-api/00-http-api/01-http-request-response.md) [01](../02-backend-api/02-nestjs/01-request-lifecycle.md) |
| Vì sao request bị từ chối? | CORS, auth, validation, guard, rate limit | [06](../01-web-frontend/00-web-foundations/06-cors.md) [03](../02-backend-api/02-nestjs/03-validation-errors.md) [04](../02-backend-api/02-nestjs/04-guards-interceptors.md) |
| Vì sao một endpoint chậm làm cả server chậm? | event loop blocking | [01](../02-backend-api/01-nodejs/01-node-runtime-concurrency.md) [04](../02-backend-api/01-nodejs/04-worker-threads-cpu.md) |
...
| Dữ liệu được lưu bền vững thế nào? | transaction, WAL, fsync | [06](../03-database/01-postgresql/06-wal-durability-backup.md) |
| Vì sao hai request đồng thời làm sai dữ liệu? | transaction, MVCC, isolation, lock | [01](../03-database/01-postgresql/01-transaction-isolation.md) [05](../03-database/01-postgresql/05-locking-deadlock.md) |
| Vì sao query nhanh/chậm? | index, query planner, EXPLAIN | [02](../03-database/01-postgresql/02-index-query-plan.md) [08](../03-database/01-postgresql/08-explain-analyze-workflow.md) |
| Vì sao table phình to dù đã xoá dữ liệu? | MVCC, dead tuple, vacuum | [04](../03-database/01-postgresql/04-mvcc-vacuum.md) |
...
| Khi process/container chết, hệ thống hồi phục thế nào? | restart policy, desired state | [04](../04-infrastructure/04-kubernetes/04-why-kubernetes.md) [01](../04-infrastructure/04-kubernetes/01-pod-deployment-service.md) |
| Vì sao deploy gây downtime? | readiness, rolling update, graceful shutdown | [02](../04-infrastructure/04-kubernetes/02-health-readiness-liveness.md) [05](../02-backend-api/01-nodejs/05-graceful-shutdown.md) |
