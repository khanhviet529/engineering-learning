...
|---|---|---|
| Dữ liệu nhảy về giá trị cũ sau khi click nhanh | React | [Async race condition](../01-web-frontend/02-react/03-async-race-condition.md) |
| Giá trị trong callback là giá trị của lần render trước | JavaScript closure | [Execution context & closure](../01-web-frontend/01-javascript-typescript/03-execution-context-closure.md) · [Effects & lifecycle](../01-web-frontend/02-react/02-effects-lifecycle.md) |
| DB đã đổi nhưng trang Next.js vẫn cũ | Next.js cache | [Data fetching & cache](../01-web-frontend/03-nextjs/03-data-fetching-cache.md) |
...
| Số tiền / tồn kho sai sau khi có nhiều người dùng | Database concurrency | [Transaction isolation](../03-database/01-postgresql/01-transaction-isolation.md) |
| Input mất chữ khi list re-order | React key | [Reconciliation & keys](../01-web-frontend/02-react/05-reconciliation-keys.md) |
...
| Type nói có mà runtime `undefined` | TS ↔ runtime | [TypeScript ↔ runtime boundary](../01-web-frontend/01-javascript-typescript/02-typescript-runtime-boundary.md) |
...
| Gõ vào input bị lag | React render | [React performance](../01-web-frontend/02-react/12-performance.md) |
| Một endpoint chậm, các endpoint khác cũng chậm theo | Node event loop bị block | [Node runtime & concurrency](../02-backend-api/01-nodejs/01-node-runtime-concurrency.md) · [Worker threads & CPU](../02-backend-api/01-nodejs/04-worker-threads-cpu.md) |
| Nhanh với 1k dòng, chết với 1M dòng | Missing index / seq scan | [Index & query plan](../03-database/01-postgresql/02-index-query-plan.md) |
...
| Latency p50 ổn nhưng p99 rất tệ | Queue / pool contention | [Latency & throughput](../05-cross-cutting/performance/01-latency-throughput-bottleneck.md) · [Connection pool](../03-database/01-postgresql/03-connection-pool.md) |
| Chậm dần theo thời gian, restart lại nhanh | Memory leak / bloat | [Memory & GC](../01-web-frontend/01-javascript-typescript/05-memory-gc.md) · [MVCC & vacuum](../03-database/01-postgresql/04-mvcc-vacuum.md) |
| Chậm chỉ khi deploy version mới | Cold cache / cold start | [Cache patterns](../03-database/02-redis/03-cache-patterns.md) · [Rollout & rollback](../04-infrastructure/04-kubernetes/07-rollout-rollback.md) |
...
| `too many connections` từ PostgreSQL | Pool sizing | [Connection pool](../03-database/01-postgresql/03-connection-pool.md) |
| Service trong K8s không resolve | Service discovery | [Ingress & service discovery](../04-infrastructure/04-kubernetes/06-ingress-service-discovery.md) |

### "Nó bị từ chối"

| Triệu chứng | Tầng khả nghi | Note |
|---|---|---|
| 400 với payload trông hợp lệ | Validation pipe | [Validation & errors](../02-backend-api/02-nestjs/03-validation-errors.md) |
...
| Triệu chứng | Tầng khả nghi | Note |
|---|---|---|
| Effect chạy 2 lần trong dev | StrictMode | [Effects & lifecycle](../01-web-frontend/02-react/02-effects-lifecycle.md) |
...
| Request mất khi deploy | Không graceful shutdown | [Graceful shutdown](../02-backend-api/01-nodejs/05-graceful-shutdown.md) |
...
| Triệu chứng | Tầng khả nghi | Note |
|---|---|---|
| Pod `CrashLoopBackOff` | Config / probe / lỗi khởi động | [Debugging K8s](../04-infrastructure/04-kubernetes/10-debugging-k8s.md) |
...
| JS heap out of memory | Leak | [Process & memory](../02-backend-api/01-nodejs/03-process-memory.md) |
...
| Deploy xong toàn bộ 503 | Readiness sai | [Readiness & liveness](../04-infrastructure/04-kubernetes/02-health-readiness-liveness.md) |
| `deadlock detected` | Thứ tự lock | [Locking & deadlock](../03-database/01-postgresql/05-locking-deadlock.md) |
| App trắng khi có một lỗi render | Không có error boundary | [Error boundaries & Suspense](../01-web-frontend/02-react/09-error-boundaries-suspense.md) |
...
| Cần đọc query plan | [EXPLAIN ANALYZE workflow](../03-database/01-postgresql/08-explain-analyze-workflow.md) |
