...
area: backend
prerequisites:
  - ../../../01-web-frontend/01-javascript-typescript/async-concurrency/01-event-loop-async.md
related:
  - ../runtime-io/02-worker-threads-cpu.md
...
Chi tiết: [Worker threads & CPU](../runtime-io/02-worker-threads-cpu.md).
...
Trong Kubernetes, thường **không** dùng cluster: chạy nhiều pod, mỗi pod một process. Lý do: scheduler và metric của K8s hoạt động ở mức pod, và một pod một process dễ quan sát hơn. Xem [Scheduling & resources](../../../04-infrastructure/04-kubernetes/scheduling-reliability/02-scheduling-resources.md).
...
4. **Latency tăng đều theo tải nhưng CPU không cao** → nghi thread pool cạn (fs/crypto) hoặc connection pool cạn (DB). Xem [Connection pool](../../../03-database/01-postgresql/fundamentals/02-connection-pool.md).
...
- **Graceful shutdown** để không mất request khi deploy. Xem [Graceful shutdown](../production/02-graceful-shutdown.md).
...
- [Event loop & async](../../../01-web-frontend/01-javascript-typescript/async-concurrency/01-event-loop-async.md) — cơ chế nền
- [Worker threads & CPU](../runtime-io/02-worker-threads-cpu.md) — song song thật
- [Streams & buffers](../runtime-io/01-streams-buffers.md) — xử lý dữ liệu lớn không chặn
- [Process & memory](../production/01-process-memory.md) — giới hạn heap
- [Graceful shutdown](../production/02-graceful-shutdown.md)
- [Connection pool](../../../03-database/01-postgresql/fundamentals/02-connection-pool.md) — một loại cạn tài nguyên khác
