...
4. **`destroy()` trong graceful shutdown** — worker không tự thoát. Xem [Graceful shutdown](05-graceful-shutdown.md).
...
1. **Xác nhận đúng là CPU-bound**: đo [event loop lag](01-node-runtime-concurrency.md). Lag cao + CPU cao = CPU-bound. Lag cao + CPU thấp = blocking I/O đồng bộ.
...
- [Node runtime & concurrency](01-node-runtime-concurrency.md) — vì sao CPU-bound chặn tất cả
- [Process & memory](03-process-memory.md) — memory của worker
- [Graceful shutdown](05-graceful-shutdown.md) — `terminate()` worker
