...
Lợi ích cụ thể chứ không phải lý thuyết: một job render PDF ăn 100% CPU **không** làm event loop của API bị đói, và bạn scale worker theo độ dài hàng đợi thay vì theo số request. Xem [Worker threads & CPU](../01-nodejs/04-worker-threads-cpu.md) cho trường hợp phải giữ trong cùng process.
...
12. Event loop bị chặn → health check timeout → K8s restart pod → job bị giết → retry → lặp lại. Xem [Node runtime & concurrency](../01-nodejs/01-node-runtime-concurrency.md).
</details>
...
- [Worker threads & CPU](../01-nodejs/04-worker-threads-cpu.md) — khi phải ở cùng process
- [Graceful shutdown](../01-nodejs/05-graceful-shutdown.md) — đóng worker đúng cách
