...
Bốn dòng đó là toàn bộ lý do transaction phải **ngắn**. Một transaction 3 giây không chỉ chậm — nó giữ 1/10 khả năng đồng thời của app, khoá row mà request khác đang chờ, và làm autovacuum không dọn được. Xem [Connection pool](../../03-database/01-postgresql/03-connection-pool.md) và [MVCC & vacuum](../../03-database/01-postgresql/04-mvcc-vacuum.md).
...
Xem [Index & query plan](../../03-database/01-postgresql/02-index-query-plan.md).
...
Đánh đổi: `FOR UPDATE` đơn giản nhưng tạo hàng đợi và có thể deadlock; optimistic lock không khoá gì nhưng đẩy việc retry sang client. Xem [Locking & deadlock](../../03-database/01-postgresql/05-locking-deadlock.md).
...
- **Đọc từ replica cho báo cáo**, nhưng nhớ replication lag: ghi rồi đọc ngay từ replica có thể không thấy dữ liệu vừa ghi. Xem [Replication & scaling](../../03-database/01-postgresql/09-replication-scaling.md).
...
- [Transaction isolation](../../03-database/01-postgresql/01-transaction-isolation.md) — mức isolation và anomaly
- [Locking & deadlock](../../03-database/01-postgresql/05-locking-deadlock.md) — khoá, hàng đợi, deadlock
- [Connection pool](../../03-database/01-postgresql/03-connection-pool.md) — vì sao transaction dài làm cạn pool
- [MVCC & vacuum](../../03-database/01-postgresql/04-mvcc-vacuum.md) — vì sao transaction dài làm phình bảng
- [Index & query plan](../../03-database/01-postgresql/02-index-query-plan.md) — N+1 và query chậm
