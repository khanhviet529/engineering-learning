...
TypeScript không tồn tại lúc runtime. Điều này đã được nói ở [TypeScript ↔ runtime boundary](../../../01-web-frontend/01-javascript-typescript/typescript/01-runtime-boundary.md); ở backend nó chuyển từ khó chịu thành nguy hiểm:
...
- Service không thay được constraint: giữa `SELECT` kiểm tra trùng và `INSERT` có một khe thời gian, và hai request đồng thời chui lọt qua khe đó. Xem [Transaction isolation](../../../03-database/01-postgresql/transactions-concurrency/01-transaction-isolation.md).
...
Có `@MaxLength` ở mọi string là kỷ luật rẻ: nếu không, một field `description` không giới hạn là một đường tới OOM. Xem [Process & memory](../../01-nodejs/production/01-process-memory.md).
...
- [TypeScript ↔ runtime boundary](../../../01-web-frontend/01-javascript-typescript/typescript/01-runtime-boundary.md) — gốc của vấn đề
...
- [Forms (React)](../../../01-web-frontend/02-react/behavior/10-forms.md) — phía client của cùng một DTO
