...
area: infra
prerequisites:
  - ../fundamentals/01-why-kubernetes.md
...
Với nhiều service HTTP, **Ingress** (một LB, routing theo host/path) rẻ và linh hoạt hơn nhiều LoadBalancer Service. Xem [Ingress & service discovery](02-ingress-service-discovery.md).
...
Nếu bước 2 cho thấy **có pod nhưng `READY 0/1`**, nguyên nhân khác hẳn: readiness probe fail. Xem [Readiness & liveness](../scheduling-reliability/01-health-readiness-liveness.md).
...
2. **Nếu `<none>`**: `kubectl get pods -l <selector>` — có pod nào khớp không?
   ...
   - Có pod nhưng `0/1` → **readiness probe fail**. Xem [Readiness & liveness](../scheduling-reliability/01-health-readiness-liveness.md).
...
- [Vì sao cần Kubernetes](../fundamentals/01-why-kubernetes.md) — đọc trước
- [Readiness & liveness](../scheduling-reliability/01-health-readiness-liveness.md) — vì sao pod không ready
- [Config, secrets & resources](../operations/01-config-secrets-resources.md) — ConfigMap, Secret, requests/limits
- [Ingress & service discovery](02-ingress-service-discovery.md) — lộ service ra ngoài
- [Rollout & rollback](03-rollout-rollback.md) — ReplicaSet và rolling update
- [Debugging Kubernetes](../operations/02-debugging-k8s.md) — quy trình đầy đủ
