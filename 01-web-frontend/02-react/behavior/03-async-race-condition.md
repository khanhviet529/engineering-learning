...
Vì sao đúng: mỗi lần effect chạy tạo một biến `ignore` **riêng** (closure). Khi `userId` đổi, cleanup của lần trước set `ignore = true` cho closure *đó*, nên response A không ghi được nữa. Đây là [closure](../../01-javascript-typescript/fundamentals/01-execution-context-closure.md) được dùng đúng mục đích.
...
- [Promise & concurrency](../../01-javascript-typescript/async-concurrency/02-promise-concurrency.md) — nền async
