...
Bốn bug, ba nguyên nhân: hiểu sai mục đích của effect, hiểu sai dependency, và [stale closure](../../01-javascript-typescript/fundamentals/01-execution-context-closure.md).
...
4. **Leak** → `getEventListeners(window)` trong Chrome console; hoặc heap snapshot. Xem [Memory & GC](../../01-javascript-typescript/runtime-behavior/01-memory-gc.md).
...
- [Closure](../../01-javascript-typescript/fundamentals/01-execution-context-closure.md) — cơ chế stale closure
- [Memory & GC](../../01-javascript-typescript/runtime-behavior/01-memory-gc.md) — leak từ thiếu cleanup
