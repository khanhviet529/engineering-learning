...
Không có `try/catch` nào ở controller. Exception filter là boundary duy nhất: nó map `AppError.status` → HTTP status, log theo mức phù hợp với loại lỗi. Xem [Validation & errors](../../02-backend-api/02-nestjs/03-validation-errors.md).
...
- [Promise & concurrency](04-promise-concurrency.md) — unhandled rejection
...
- [Error boundaries & Suspense](../02-react/09-error-boundaries-suspense.md) — boundary ở frontend
- [State → render](../02-react/01-state-render.md) — vì sao reference quan trọng
