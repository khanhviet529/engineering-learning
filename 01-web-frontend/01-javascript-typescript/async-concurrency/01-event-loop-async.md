...
Đây là behavior nền cho: [Promise](02-promise-concurrency.md), [React effect](../../02-react/behavior/02-effects-lifecycle.md), [race condition](../../02-react/behavior/03-async-race-condition.md), [Node concurrency](../../../02-backend-api/01-nodejs/fundamentals/01-runtime-concurrency.md).
...
Trong browser thì đơn giản hơn nhưng có thêm bước render. Chi tiết Node: [Node runtime & concurrency](../../../02-backend-api/01-nodejs/fundamentals/01-runtime-concurrency.md).
...
- **Ở Node: một handler CPU-bound làm chậm mọi request** — vì cùng một thread. Xem [Worker threads](../../../02-backend-api/01-nodejs/runtime-io/02-worker-threads-cpu.md).
...
- **`await` trong vòng lặp khi có thể song song** — biến 10 request 100ms thành 1 giây thay vì 100ms. Xem [Promise & concurrency](02-promise-concurrency.md).
...
- [Worker threads & CPU](../../../02-backend-api/01-nodejs/runtime-io/02-worker-threads-cpu.md) — khi cần song song thật
- [React effects](../../02-react/behavior/02-effects-lifecycle.md) — effect chạy ở đâu trong vòng này
