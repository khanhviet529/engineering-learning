...
Điều kiện để retry đúng: **hàm `fn` phải chạy lại được từ đầu**. Nghĩa là không có side effect ngoài database bên trong nó — không gửi email, không gọi API. Xem [Database & transactions](../../02-backend-api/02-nestjs/06-database-integration-transactions.md).
...
- **Transaction dài** → giữ connection, giữ lock, chặn vacuum. Xem [MVCC & vacuum](04-mvcc-vacuum.md).
- **Network call trong transaction** → transaction dài, và pool cạn dưới tải.
- **Khoá theo thứ tự khác nhau** → deadlock. Xem [Locking & deadlock](05-locking-deadlock.md).
...
- [MVCC & vacuum](04-mvcc-vacuum.md) — cơ chế tạo ra snapshot
- [Locking & deadlock](05-locking-deadlock.md) — `FOR UPDATE`, hàng đợi, deadlock
- [Connection pool](03-connection-pool.md) — vì sao transaction dài làm cạn pool
- [Constraints & invariants](../03-data-modeling/01-constraints-invariants.md) — phòng tuyến cuối
- [Database & transactions (NestJS)](../../02-backend-api/02-nestjs/06-database-integration-transactions.md) — ranh giới transaction trong code
