...
Nghĩa là pool size của một instance Node không cần lớn. Nó bị giới hạn bởi số request đồng thời mà instance đó xử lý, và bởi thời gian mỗi query. Xem [Node runtime & concurrency](../../../02-backend-api/01-nodejs/fundamentals/01-runtime-concurrency.md).
...
Quy tắc: **không bao giờ gọi mạng bên trong transaction.** Xem [Database & transactions](../../../02-backend-api/02-nestjs/behavior/06-database-integration-transactions.md).
...
`idle in transaction` là tín hiệu quan trọng nhất trong bảng này: nó nghĩa là app đã `BEGIN` rồi đi làm việc khác. Mỗi dòng như vậy giữ một connection **và** giữ một snapshot chặn vacuum. Xem [MVCC & vacuum](../transactions-concurrency/02-mvcc-vacuum.md).
...
4. **Nếu nhiều `active` và CPU DB cao** → vấn đề là query, không phải pool. Đi sang [Index & query plan](../indexes-query-planning/01-index-query-plan.md).
...
- [Transaction isolation](../transactions-concurrency/01-transaction-isolation.md) — transaction dài và tranh chấp
- [MVCC & vacuum](../transactions-concurrency/02-mvcc-vacuum.md) — vì sao `idle in transaction` chặn vacuum
- [Index & query plan](../indexes-query-planning/01-index-query-plan.md) — query nhanh giảm áp lực lên pool
- [Node runtime & concurrency](../../../02-backend-api/01-nodejs/fundamentals/01-runtime-concurrency.md) — một event loop, một pool
- [Database & transactions (NestJS)](../../../02-backend-api/02-nestjs/behavior/06-database-integration-transactions.md) — transaction ngắn
- [Database performance](../../../05-cross-cutting/performance/04-database-performance.md) — đo ở tầng hệ thống
- [Autoscaling](../../../04-infrastructure/04-kubernetes/scheduling-reliability/03-autoscaling.md) — scale app trong giới hạn DB
